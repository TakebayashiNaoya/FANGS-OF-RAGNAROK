...
namespace fang::rhi
{
	...
	void CommandList::SetPipeline(PipelineHandle pipeline)
	{
		...
		const PipelinePool::Entry& entry = m_device->m_impl->pipelines.Get(pipeline);
	...
	void CommandList::SetVertexBuffer(BufferHandle buffer)
	{
		...
		const BufferPool::Entry& entry = m_device->m_impl->buffers.Get(buffer);
	...
	void CommandList::SetTexture(TextureHandle texture)
	{
		...
		const TexturePool::Entry&         entry      = impl.textures.Get(texture);
		const D3D12_GPU_DESCRIPTOR_HANDLE descriptor = impl.shaderVisibleHeap.GetGPUHandle(entry.descriptorIndex);
