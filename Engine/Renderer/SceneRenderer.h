 ...
 * @brief View とカリング、フレーム定数の一本化を受け持ち、ScenePass を RenderGraph へ宣言する。
 */
#pragma once
...
namespace fang
{
	...
	/**
	 * @brief View ごとにフラスタムカリングとフレーム定数（b1）をまとめ、ScenePass を宣言する。
	 * @details MeshRenderer は借用ポインタで持つ。1 フレームに何度も AddView / Submit / AddPasses を
	 *          呼び直す前提で、持ち物は View 数（4）ぶんの固定長配列 ➡ 定常状態のヒープ確保は 0。
	 * @threading AddView / Submit / AddPasses / Reset はメインスレッドのみ。記録は RenderGraph のジョブで
	 *            走り、View ごとに書き込み先（自分の b1 バッファと描画数のスロット）が分かれるので競合しない。
	...
	public:
		...
		/**
		 * @brief View 数ぶんの b1 用動的定数バッファを確保する。
		...
		/** @brief 確保した定数バッファを解放する。二重に呼んでも安全。 */
		void Shutdown(rhi::GraphicsDevice& device);

		/** @brief View・Submit の控え・描画数をフレームの頭で巻き戻す。 */
		void Reset();
		 ...
		 *          MeshFrameConstants を 1 フレームに 1 回だけ書く。
		 */
	...
	private:
		...
		/**
		 * @brief その View の Submit 控えを視錐台で絞り、生き残った分を MeshRenderer::Draw へ渡す。
		...
		Frustum m_frustums[MAX_VIEW_COUNT]; /**< AddView が抽出した 6 平面。View ごと。 */
