...
namespace fang
{
	 ...
	 * @details 引数は Submit がここへ写す。
	 *          呼び出し側のメモリを持ち続けないので、数フレーム生きるジョブでも積んだ側が先に消えてよい。
	 */
	...
	/**
	 * @brief 実行に参加する 1 人分。0 番はメインスレッドの分で、常駐スレッドを持たない。
...
#pragma warning(pop)
	...
	bool JobSystem::Initialize(const JobSystemDesc& desc)
	{
		...
		if (workerCount == 0)
		{
			workerCount = GetUsableCoreCount();
		}
		...
		// 名前を付けたヒープから取ると、メモリの内訳にそのまま出て、返し忘れもリーク報告が捕まえる。
		m_heap = &CreateHeap("JobSystem");

		// ジョブは実行中に増えない。ここで全部確保して、以降は空きリストで回すだけにする。
		void* poolMemory = m_heap->Allocate(sizeof(Job) * JOB_POOL_CAPACITY, CACHE_LINE_SIZE);
		...
		// 配列形の new を使わないのは、要素数のクッキーが先頭に付く型で
		// 先頭要素がキャッシュライン境界に載る保証が無いため。
		void* workerMemory = m_heap->Allocate(sizeof(Worker) * workerCount, CACHE_LINE_SIZE);
		if (workerMemory == nullptr)
		{
			FANG_LOG_ERROR(Core, "ワーカーを確保できなかった ({} 人)", workerCount);
			ReleaseResources();
			return false;
		}

		m_workers = static_cast<Worker*>(workerMemory);
	...
	void JobSystem::Wait(JobCounter& counter)
	{
		...
		while (!counter.IsComplete())
		{
			...
			// 仕事も無く、まだ終わっていない。寝る先はカウンタでなく JobSystem のチケット。
			// カウンタは完了と同時に壊されてよいので、そこで寝ると死んだ番地で寝続けることになる。
	...
	{
		FANG_ASSERT(workerIndex < m_workerCount, "ワーカー番号 {} は範囲外", workerIndex);
	...
	void JobSystem::ReleaseResources()
	{
		if (m_workers != nullptr)
		{
			for (uint32_t i = 0; i < m_workerCount; ++i)
			...
			m_heap->Deallocate(m_workers);
			m_workers = nullptr;
		}

		if (m_jobPool != nullptr)
		{
			for (uint32_t i = 0; i < JOB_POOL_CAPACITY; ++i)
			{
				m_jobPool[i].~Job();
	...
	bool JobSystem::TryTakeJob(uint32_t workerIndex, uint32_t& outJobIndex)
	{
		...
		const uint32_t startIndex = worker.NextRandom() % m_workerCount;
		for (uint32_t attempt = 0; attempt < m_workerCount; ++attempt)
		{
			const uint32_t victimIndex = (startIndex + attempt) % m_workerCount;
	...
	void JobSystem::DispatchPendingList(uint32_t firstJobIndex, uint32_t workerIndex)
	{
		...
		while (jobIndex != INVALID_JOB_INDEX)
		{
			...
			// 依存の解けていないジョブを引き取ってしまうことがある。
			// その分は保留リストへ戻す（Submit と同じく、戻した直後の 0 到達は自分で拾い直す）。
	...
	{
		// 増やすのと完了フラグを下ろすのを 1 回の CAS で済ませる。
		// 別々にすると、下ろした後に前の代の引き取り役が完了を立て直し、走っているジョブを置いて Wait が抜ける。
		// 通し番号も一緒に進める。
		// これがないと「0 ➡ 積む ➡ また 0」で状態語が元の値に戻り、公開しようとしている引き取り役の
		// CAS が通ってしまい、その間に積まれた保留ジョブが置き去りになる。
	...
	{
		// 減らすのと「引き取り役を名乗る」のも 1 回の CAS で済ませる。
		// 別々にすると、名乗るまでの間に次の Submit と完了が走り抜けて、引き取り役が 2 人になる。
		uint64_t state           = counter.m_state.load(std::memory_order_relaxed);
		uint64_t nextState       = 0;
