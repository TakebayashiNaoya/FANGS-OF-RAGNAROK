...
namespace fang
{
	 ...
	 * @param workerIndex 実行中のワーカー番号。0 はメインスレッド。ワーカーごとの配列を引くのに使う。
	 */
	...
	{
		uint32_t workerCount = 0; /**< 実行に参加する人数（メイン込み）。0 なら GetUsableCoreCount()。 */
	};
	 ...
	 * @details ワーカーごとに両端キューを持ち、自分の分が尽きたら他人から奪う。
	 *          メインスレッドは常駐スレッドを持たず、Wait / ParallelFor の間だけワーカー番号 0 として参加する。
	 * @threading Submit / Wait はメインスレッドとジョブの中から呼べる。
	 *            それ以外のスレッドは自分のキューを持たないので、Submit がその場で実行する縮退になる。
	 *            Initialize / Shutdown はメインスレッドのみ。
	 */
	class JobSystem
	{
	public:
		...
		static constexpr uint32_t MAX_WORKER_COUNT  = 32;   /**< 実行に参加できる人数の上限。メイン込み。 */
		...
		/** @brief 実行に参加する人数。メインを含む。ワーカー番号は [0, この値)。 */
		[[nodiscard]] FANG_FORCEINLINE uint32_t GetWorkerCount() const { return m_workerCount; }

		/** @brief 常駐スレッドの本数。メインはスレッドを持たないので GetWorkerCount() − 1。 */
		[[nodiscard]] FANG_FORCEINLINE uint32_t GetWorkerThreadCount() const
		{
			return m_workerCount > 0 ? m_workerCount - 1 : 0;
		}
	...
	public:
		 ...
		 * @param desc workerCount が 0 なら使えるコア数から決める。
		 * @return 失敗したら false。スレッドは 1 本も残らない。
		 */
		...
		/** @brief 生きている常駐スレッドの数。Shutdown 後は 0。 */
	...
	private:
		...
		HeapAllocator* m_heap    = nullptr; /**< 自分専用のヒープ。確保するのは Initialize の中だけ。 */
		Worker*        m_workers = nullptr; /**< 0 番はメインスレッドの分。スレッドを持たない。 */
		Job*           m_jobPool = nullptr; /**< 固定長。実行中はここから増えも減りもしない。 */

		uint32_t m_workerCount = 0; /**< 実行に参加する人数。メイン込み。 */
		 ...
		 * @details 眠っているワーカーはこれに atomic::wait する。
		 *          bool の起床フラグだと「無いことを確かめてから寝るまで」の隙間で起こし損ねるが、
		 *          値が変わっていれば wait は素通りする。
		 */
		std::atomic<uint32_t> m_submitTicket{ 0 };

		/**
		 * @brief どれかのカウンタが完了を公開するたびに増える通し番号。
		 * @details Wait はカウンタでなくこれに atomic::wait する。
		 *          カウンタは完了と同時に壊されてよい約束なので、そこで寝ると死んだ番地で寝続けることになる。
		 */
